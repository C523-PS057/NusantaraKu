...
<form action="{{ route('budaya.index') }}" method="GET">
    <div class="input-group mb-3 dashboard__search">
        <div class="col-md-5">
            <input name="search" type="text" class="form-control" placeholder="Cari Kategori Budaya Disini..."
        ...
        <div class="dashboard-admin__grid">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    <a href="{{ $item->id === 1 ? '/masakan-khas' : ($item->id === 2 ? '/alat-musik' : ($item->id === 3 ? '/pakaian-adat' : ($item->id === 4 ? '/rumah-adat' : ($item->id === 5 ? '/tari-tradisional' : '#')))) }}"
                        class="btn btn-primary" target="_blank" rel="noopener noreferrer">Lihat Semua</a>
                </div>
            </div>
            @endforeach
        </div>
