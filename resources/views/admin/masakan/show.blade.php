...
<div class="container">
    <div class="row mb-5">
        <div class="col-md-8">
            <h2 class="mb-3">{{ $data->masakan_name }}</h2>
            <div class="mb-3">
            <a href="{{ route('masakan.index') }}" class="btn btn-success"><i class="bi bi-arrow-left-circle"></i>
                Back To All Of Masakan Khas</a>
            <a href="{{ route('masakan.edit',$data->id) }}" class="btn btn-warning"><i class="bi bi-pencil-fill"></i> Edit</a>
            <a href="{{ route('province.show',$data->province->id) }}" class="btn btn-primary"><i class="bi bi-pencil-fill"></i>Province</a>
            <form class="d-inline" action="{{ route('masakan.destroy',$data->id) }}" method="POST">
                @csrf
                @method('delete')
