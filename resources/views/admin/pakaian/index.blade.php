...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
  ...
  <div class="input-group mb-3">
    <div class="col-md-5">
        <input name="search" type="text" class="form-control" placeholder="Cari Pakaian Adat.." aria-label="search" aria-describedby="basic-addon2" value="{{ request('search') }}">
    </div>
                            ...
                            <div class="detail__item">
                                ...
                                <div class="detail__item-content">
                                    ...
                                    <div class="detail__item-buttons">
                                      <a href="/detail-pakaian/{{ $item->id }}" class="btn btn-outline" target="_blank" rel="noopener noreferrer"><i class="ri-eye-line"></i> Lihat</a>
                                      <a href="{{ route('pakaian.edit',$item->id) }}" class="btn btn-outline"><i class="ri-edit-line"></i>Ubah</a>
                                      <form action="{{ route('pakaian.destroy',$item->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-primary" style="width: 100%" onclick="return confirm('Yakin hapus data pakaian adat ini?')"><i class="ri-delete-bin-line"></i> Hapus</button>
                                    </form>
                                    </div>
                                </div>
                            </div>
  ...
  @else
    <div class="d-flex justify-content-center my-2">
        <div class=""><h2>Tidak ada Pakaian Adat yang ditemukan</h2></div>
    </div>
